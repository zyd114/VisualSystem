        ...
        fetch('./data_Sex_Age_SQ_SD.json')
            ...
            .then(data => {
                ...
                data.forEach(entry => {
                    ...
                    if (Data[gender]) {
                        Data[gender].push({ age, sleepDuration });
                    }
                });
                // 提取年龄信息并去重排序
                ...
                const maleData = ages.map(age => {
                    ...
                    // 计算平均值
                    if (entries.length > 0) {
                        const totalSD = entries.reduce((sum, entry) => sum + parseFloat(entry.sleepDuration), 0);
                        const avgSD = totalSD / entries.length;
                        return parseFloat(avgSD.toFixed(2));    // 保留两位小数
                    }
                    // 如果没有数据，返回 0
                    return 0;
                });
                ...
                const femaleData = ages.map(age => {
                    ...
                    if(entries.length > 0) {
                        const totalSD = entries.reduce((sum, entry) => sum + parseFloat(entry.sleepDuration), 0);
                        const avgSD = totalSD / entries.length;
                        return parseFloat(avgSD.toFixed(2));
                    }
                    return 0;
                });
                ...
                const chartOption = {
                    title: {
                        text: 'Gender, Age, and Average Sleep Duration',
                        left: 'center',
                        top: 0
                    },
                    tooltip: {
...
export default Data_Sex_Age_SD;